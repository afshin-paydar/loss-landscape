...
Stage 1 - Warm-up:       Train W only (freeze alpha, beta)
...
Usage:
  python train_joint.py --model resnet56 \
    --pretrained cifar10/trained_nets/model_300.t7 \
...
def train_warmup(net, trainloader, testloader, args):
    """Warm-up: train W only with CE loss."""
    print("\n" + "=" * 60)
    print("STAGE 1: Warm-up (train W only)")
    print("=" * 60)
...
def train_joint_search(net, trainloader, testloader, args):
    ...
    # Separate optimizer groups: different LRs for W vs alpha/beta
    w_params = [p for n, p in net.named_parameters() 
                if 'pruning_alphas' not in n and 'bitwidth_betas' not in n and p.requires_grad]
    a_params = list(net.pruning_alphas.parameters())
    b_params = list(net.bitwidth_betas.parameters())
    
    optimizer = optim.Adam([
        {'params': w_params, 'lr': args.joint_lr_w},
    ...
    with torch.no_grad():
        # Temporarily set all gates to 1, all bits to 32 to get baseline size
        baseline_size = 0
    ...
    print(f"  Baseline size: {baseline_size} bits, S_max: {S_max:.0f} bits ({args.S_max_ratio:.0%})")
    ...
    for epoch in range(args.joint_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            size_penalty = lambda_s * torch.clamp(model_size - S_max, min=0)
        ...
        # Dual variable update every 5 epochs
        if (epoch + 1) % 5 == 0:
            with torch.no_grad():
                current_size = net.compute_model_size().item()
                constraint_violation = current_size - S_max
        ...
        print(f"  Epoch {epoch+1}/{args.joint_epochs} | "
              f"Task: {train_loss/total:.4f} | Lagr: {epoch_lagrangian/total:.4f} | "
              f"Acc: {test_acc:.2f}% | Sparsity: {sparsity:.2%} | "
              f"tau: {tau:.3f} | lambda_s: {lambda_s:.4f}")
...
def main():
    ...
    parser.add_argument('--joint_lr_w', default=0.001, type=float, help='LR for weights')
    parser.add_argument('--joint_lr_alpha', default=0.01, type=float, help='LR for pruning gates')
    parser.add_argument('--joint_lr_beta', default=0.01, type=float, help='LR for bit-width logits')
    parser.add_argument('--lambda_s', default=0.1, type=float, help='initial Lagrange multiplier')
    parser.add_argument('--gamma', default=0.01, type=float, help='sparsity regularization weight')
    ...
    parser.add_argument('--tau_end', default=0.1, type=float, help='final Gumbel temperature')
    parser.add_argument('--dual_lr', default=0.01, type=float, help='dual variable learning rate')
    parser.add_argument('--gate_threshold', default=0.01, type=float)
